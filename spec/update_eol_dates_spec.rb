...
RSpec.describe 'update_eol_dates' do
  ...
  describe '#fetch_eol_data' do
    ...
    context 'with failed API response' do
      ...
      it 'returns nil and warns on HTTP error' do
        expect { fetch_eol_data('invalid-product') }.to output(/Failed to fetch data/).to_stderr
        expect(fetch_eol_data('invalid-product')).to be_nil
      end
    end
    ...
    context 'with network error' do
      ...
      it 'returns nil and warns on exception' do
        expect { fetch_eol_data(product) }.to output(/Error fetching data/).to_stderr
        expect(fetch_eol_data(product)).to be_nil
      end
    end
  end
  ...
  describe '#update_os_data' do
    ...
    it 'updates OS data from API' do
      result = update_os_data(os_name, product_id, current_data)
    ...
    it 'preserves versions not in API' do
      ...
      result = update_os_data(os_name, product_id, current_with_extra)
      expect(result['0.5']).to eq('2015-01-01')
    end

    it 'returns current data when product_id is nil' do
      result = update_os_data(os_name, nil, current_data)
  ...
  describe '#handle_amazon_linux' do
    ...
    it 'combines data from all Amazon Linux versions' do
      result = handle_amazon_linux(current_data)
      expect(result).to be_a(Hash)
    ...
    it 'preserves versions not in any API' do
      ...
      result = handle_amazon_linux(current_with_extra)
      expect(result['2017.03']).to eq('2020-12-31')
    end

    it 'handles API failures gracefully' do
      allow(self).to receive(:fetch_eol_data).and_return(nil)
      expect { handle_amazon_linux(current_data) }.not_to raise_error
    end
  end
  ...
  describe '#handle_centos' do
    ...
    it 'combines data from both CentOS and CentOS Stream APIs' do
      result = handle_centos(current_data)
      expect(result).to be_a(Hash)
    ...
    it 'preserves versions not in any API' do
      ...
      result = handle_centos(current_with_extra)
      expect(result['4']).to eq('2012-02-29')
    end

    it 'handles API failures gracefully' do
      allow(self).to receive(:fetch_eol_data).and_return(nil)
      expect { handle_centos(current_data) }.not_to raise_error
    end
  end
